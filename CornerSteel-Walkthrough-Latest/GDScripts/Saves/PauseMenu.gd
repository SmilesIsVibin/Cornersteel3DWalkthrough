...
var previous_clock_bus1_state: bool = true
var previous_clock_bus2_state: bool = true
var previous_clock_bus3_state: bool = true
var previous_clock_bus4_state: bool = true
...
func _process(_delta: float) -> void:
	...
	on_off()
	#only changes the light when it is being change
	lightchanges()
...
#------------------------- Bus1 -------------------------------------------------------
func light_energy_low_bus1() -> void:
	if Clock.is_bus1_on:
		if switch.size() > 0:
			var first_switch = switch[0]
			first_switch.is_switch_on = true
	else:
		if switch.size() > 0:
			var first_switch = switch[0]
			first_switch.is_switch_on = false
#------------------------- Bus2 -------------------------------------------------------
func light_energy_bus2() -> void:
	if Clock.is_bus2_on:
		if switch.size() > 0:
			var second = switch[1]
			second.is_switch_on = true
	else:
		if switch.size() > 0:
			var second = switch[1]
			second.is_switch_on = false
#------------------------- Bus3 -------------------------------------------------------
func light_energy_bus3() ->void:
	if Clock.is_bus3_on:
		if switch.size() > 0:
			var Third = switch[2]
			Third.is_switch_on = true
	else:
		if switch.size() > 0:
			var Third = switch[2]
			Third.is_switch_on = false
#------------------------- Bus4 -------------------------------------------------------
func light_energy_bus4() -> void:
	if Clock.is_bus4_on:
		if switch.size() > 0:
			var fourth = switch[3]
			fourth.is_switch_on = true
	else:
		if switch.size() > 0:
			var fourth = switch[3]
			fourth.is_switch_on = false
#only changes the light when it is being change
func lightchanges() -> void:
	if Clock.bus1 != previous_clock_bus1_state:
		light_energy_low_bus1()
		previous_clock_bus1_state = Clock.bus1
	if Clock.bus2 != previous_clock_bus2_state:
		light_energy_bus2()
		previous_clock_bus2_state = Clock.bus2
	if Clock.bus3 != previous_clock_bus3_state:
		light_energy_bus3()
		previous_clock_bus3_state = Clock.bus3
	if Clock.bus4 != previous_clock_bus4_state:
		light_energy_bus4()
		previous_clock_bus4_state = Clock.bus4
#-------------------------------------------------------------------------------------
